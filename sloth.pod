...
=head1 NAME
   ...
   sloth - slow down the speed a process runs at

=head1 SYNOPSIS

   sloth [DELAY(ms)] [PROGRAM]

=head1 DESCRIPTION

   Sloth allows you to slow down a specified application by 
   specifying (in miliseconds) the delay on the process. It slows 
   down the process by sending SIGSTOP & SIGCONT signals to the 
   process over a very fast period of time, giving the appearance 
